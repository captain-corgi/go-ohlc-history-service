# Golang Historical OHLC Price Data

Golang Service for handle OHLC data

## Overview

This service import a large amount of historical OHLC price data that in CSV files format
into RDB.

* For this demo, I use MySQL (Not included in this source code)

#### NOTE:

- In this demo, I used in-memory cache to store process status,
  as well as timer of each request. We can use Redis or Memcached to store these data.
- History of each time we upload csv, we will store in RDB, but not included in this demo.
- Unit Test contains in some package. It can be filled later
  (All module are loose coupled thanks to clean architecture structure).
- Only CSV file is able to process. Incorrect line would be ignored.
...
Instead of Echo, you can use any other http framework or even the standard `net/http` library.
